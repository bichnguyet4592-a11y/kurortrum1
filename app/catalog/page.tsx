...
export default function CatalogPage() {
  ...
  return (
    <main style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: '#f8f7f4', minHeight: '100vh' }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        @media (max-width: 768px) {
          ...
          .catalog-nav { padding: 0 20px !important; height: 70px !important; }
          .catalog-pad { padding: 20px 16px !important; }
        ...
        .filter-select, .filter-input { width: 100%; border: 1.5px solid #e5e7eb; border-radius: 10px; padding: 10px 12px; font-size: 14px; outline: none; font-family: inherit; color: #1a1a1a; transition: border-color 0.2s; background: white; }
        .filter-select:focus, .filter-input:focus { border-color: #2BAE8E; }
        .filter-label { font-size: 11px; color: #6b7280; font-weight: 600; margin-bottom: 6px; letter-spacing: 0.06em; text-transform: uppercase; display: block; }
        .toggle-btn { display: flex; align-items: center; gap: 10px; padding: 10px 14px; border-radius: 10px; border: 1.5px solid #e5e7eb; background: white; cursor: pointer; font-family: inherit; font-size: 14px; color: #374151; transition: all 0.2s; width: 100%; }
        .toggle-btn.active { border-color: #2BAE8E; background: #e6f7f3; color: #0F4C5C; font-weight: 600; }
        .toggle-dot { width: 18px; height: 18px; border-radius: 50%; border: 2px solid #d1d5db; transition: all 0.2s; flex-shrink: 0; }
      ...
      {/* NAV */}
      <nav className="catalog-nav" style={{ background: 'white', borderBottom: '1px solid #f0f0f0', padding: '0 40px', height: '80px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
        <a href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px' }}>
          {LOGO_SVG('#0F4C5C')}
          <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.15 }}>
            <span style={{ fontSize: '20px', fontWeight: 800, color: '#0F4C5C', letterSpacing: '-0.03em' }}>Курорт<span style={{ color: '#2BAE8E' }}>рум</span></span>
      ...
      <div className="catalog-pad" style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {/* ФИЛЬТРЫ */}
        <div style={{ background: 'white', border: '1px solid #f0f0f0', borderRadius: '16px', padding: '24px', marginBottom: '28px', boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
          <div className="filters-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '16px' }}>
            ...
            <div>
              <label className="filter-label">Гостей (от)</label>
              <input type="number" min={1} max={20} value={guests} onChange={e => setGuests(Number(e.target.value))} className="filter-input" placeholder="Кол-во гостей" />
            </div>

            <div>
              <label className="filter-label">Спальных мест (от)</label>
          ...
          <div style={{ marginBottom: '16px' }}>
            <label className="filter-label">Цена до: <span style={{ color: '#0F4C5C', fontWeight: 700, textTransform: 'none', letterSpacing: 0 }}>{maxPrice.toLocaleString()} ₽/ночь</span></label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <button className={`toggle-btn${parking ? ' active' : ''}`} onClick={() => setParking(!parking)}>
              <div className="toggle-dot" />
              <span>🚗 Есть парковка</span>
            </button>
            <button className={`toggle-btn${pets ? ' active' : ''}`} onClick={() => setPets(!pets)}>
              <div className="toggle-dot" />
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px', color: '#9ca3af' }}>
            ...
            <div style={{ fontSize: '18px', fontWeight: 700, color: '#374151', marginBottom: '8px' }}>Объектов не найдено</div>
            <div style={{ fontSize: '14px', marginBottom: '20px' }}>Попробуйте изменить фильтры</div>
            <button onClick={resetFilters} style={{ background: 'linear-gradient(135deg, #0F4C5C 0%, #12A387 100%)', color: 'white', border: 'none', padding: '12px 24px', borderRadius: '12px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
            ...
            <div className="catalog-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
              {properties.map(p => (
                <a key={p.id} href={`/property/${p.id}`} className="prop-card">
                  <div style={{ height: '210px', background: '#e6f7f3', overflow: 'hidden', position: 'relative' }}>
                    ...
                    <div style={{ position: 'absolute', top: '12px', left: '12px', background: 'white', borderRadius: '8px', padding: '4px 10px', fontSize: '12px', fontWeight: 700, color: '#0F4C5C' }}>{p.city}</div>
                    {p.parking && <div style={{ position: 'absolute', top: '12px', right: '12px', background: 'rgba(0,0,0,0.5)', borderRadius: '8px', padding: '4px 8px', fontSize: '12px', color: 'white' }}>🚗</div>}
                  </div>
                  <div style={{ padding: '16px 20px 20px' }}>
                    <div style={{ fontWeight: 700, fontSize: '16px', color: '#0F4C5C', marginBottom: '6px', lineHeight: 1.3 }}>{p.title}</div>
                    <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                      <span>🛏 {p.beds || p.rooms} спал.</span>
                      <span>👥 до {p.guests} гостей</span>
                      {p.pets && <span>🐾 с животными</span>}
                    </div>
